...
| `thumbInset`       | number (px)        | `10`                                                                            | Насколько центр ручки на самом деле смещён от каждого края дорожки (по умолчанию — половина ширины ручки) — используется для позиционирования `marks`, чтобы они совпадали с ручкой, а не с голым краем дорожки. Поставьте `0` для скина, у которого дорожка растянута на всю ширину без отступа (например, готовый скин **Coral** ниже). |
...
Метки позиционируются в пикселях (как и всплывающая подпись, через `ResizeObserver`), а не в
процентах — центр самой ручки на самом деле смещён от каждого края дорожки на `thumbInset` пикселей
(по умолчанию — половина ширины ручки), и без этой поправки метка на проценте 0/100 стояла бы у
голого края дорожки, а не под ручкой.
...
`thumbInset` — та же история, но наоборот: это обычная JS-опция, а не CSS-переменная, поэтому она
не подхватывает переключение скина сама по себе. Скин Coral растягивает дорожку на всю ширину через
отрицательный отступ (`::-webkit-slider-runnable-track`/`::-moz-range-track`), что отменяет
стандартное смещение ручки — поэтому у слайдера с `marks` в скине Coral нужно явно выставить
`thumbInset: 0` через `update()` при переключении (см. слайдер с метками, пункт 14, и переключатель
скина в шапке демо-страницы).
